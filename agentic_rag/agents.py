...
Answer only from retrieved_evidence. Return the required structured output.
...
def build_search_tool(retriever: BM25Retriever, *, top_k: int, min_score: float) -> BaseTool:
    """Expose deterministic retrieval as the agent's only approved tool."""

    def search(query: QueryText) -> dict:
        result = retriever.search(query, top_k=top_k, min_score=min_score)
    ...
    return StructuredTool.from_function(
        ...
        description=(
            "Search the local policy knowledge base using the user's complete question. "
